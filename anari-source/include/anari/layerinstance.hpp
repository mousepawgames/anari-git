...
public:
	...
	/// Default Constructor
	explicit LayerInstance() : transform(Transformation()) {}

	/** Constructor
	 * \param other_transform: Transformation object we are initializing with
	 */
	explicit LayerInstance(const Transformation& other_transform) : transform()
	{
		this->transform = other_transform;
	}

	/// Copy Constructor
	explicit LayerInstance(const LayerInstance& cpy) : transform(cpy.transform)
	{
	}

	/// Overloads = operator for layerinstance
	...
	/** Sets the matrix on which transformation is to be performed
	 ...
	 * \param matrix: Matrix on which transformation is to be performed
	 */
